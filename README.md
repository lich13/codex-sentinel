...
Codex Sentinel 是一个本机 macOS 菜单栏工具，用 Rust/Tauri 监控 Codex Desktop
...
Sentinel 使用一个很轻的 `lifecycle` helper 跟随 Codex Desktop。这个 helper 由
`launchd` 开机自启动；它自身常驻，但只做进程检查：
...
可见控制需要 `辅助功能` 权限；`屏幕录制` 目前只用于状态观测预留，不影响一键继续。
